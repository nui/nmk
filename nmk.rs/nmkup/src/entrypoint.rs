...
fn unzip_entrypoint(file: File, dst: impl AsRef<Path>) {
    ...
    let mut buf = Vec::new();
    gz.read_to_end(&mut buf).expect("Unable to read encoded entrypoint");
...
pub async fn install(nmk_dir: impl AsRef<Path>) -> Result<(), BoxError> {
    ...
    let uri: Uri = match target {
        Target::Amd64Linux => "https://storage.googleapis.com/nmk.nuimk.com/nmk.rs/nmk-amd64-linux-musl.gz",

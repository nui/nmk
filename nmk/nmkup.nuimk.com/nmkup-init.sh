...
# install nmkup. It just does platform detection, downloads the installer
# and runs it.
...
if [ "$KSH_VERSION" = 'Version JM 93t+ 2010-03-05' ]; then
    ...
    echo 'nmkup does not work with this ksh93 version; please try bash!' >&2
    exit 1
fi
...
main() {
    ...
    local _dir
    _dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t nmkup)"
    ...
    if [ ! -x "$_file" ]; then
        ...
        printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./nmkup-init${_ext}." 1>&2
        exit 1
    fi
